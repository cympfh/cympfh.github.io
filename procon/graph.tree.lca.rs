    ...
    fn new(tree: &Vec<Vec<usize>>, root: usize) -> Self {
        ...
        let depth: Vec<usize> = {
            ...
            while let Some((d, u)) = stack.pop() {
                if depth[u].is_some() { continue }
                depth[u] = Some(d);
                for &v in tree[u].iter() {
                    parent[v][0] = Some(u);
                    if depth[v].is_some() { continue }
                    stack.push((d + 1, v));
                }
            }
        ...
        LCA { depth: depth, parent: parent }
    }
    fn get(&self, u: usize, v: usize) -> usize {
        if self.depth[u] > self.depth[v] { return self.get(v, u) }
        let mut u = u;
        let mut v = v;
        while self.depth[u] < self.depth[v] { v = self.parent[v][0].unwrap(); }
        if u == v { return u }
        let log2n = self.parent[0].len();
        for k in (0..log2n).rev() {
            match (self.parent[u][k], self.parent[v][k]) {
                (Some(s), Some(t)) => { if s != t { u = s; v = t; } }
                _ => {}
            }
        }

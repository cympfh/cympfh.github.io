...
$\ne \emptyset$ の場合も、その中に入ってる点 $x$ に関してだけ、$C^s$ 級であればよい.
...
2. $(x_1, x_2, \ldots, x_n) \in S^n$ について、$S^n$ を次のような $2n$ コの被覆に分ける:
    - $U_i^+ = \{ x_i > 0 \}$
    - $U_i^- = \{ x_i < 0 \}$ $(i = 1,2,\ldots,n)$

明らかに $\bigcup_i U_i^+ \cup U_i^- = S^n$ である.

$\phi_i$ は次のようにする.

- $\phi_i(x_1, x_2, \ldots, (x_i > 0), \ldots, x_n) = [ x_1/x_i, x_2/x_i, \ldots, (x_i/x_i=1), \ldots, x_n/x_i ; +]$
- $\phi_i(x_1, x_2, \ldots, (x_i < 0), \ldots, x_n) = [ x_1/x_i, x_2/x_i, \ldots, (x_i/x_i=1), \ldots, x_n/x_i ; -]$

右辺は $n$ 個の数と、元の $x_i$ に関する符号を添えたものであるが、
$n$ 個の数の内の一個は定数 $1$ なので実質的に $n-1$ 次元ユークリッド空間と見なせる (同型).

3. $x \in U_i^\sigma \cap U_j^\tau$ について、
$x = (x_1, x_2, \ldots, (x_i \ne 0), \ldots, (x_j \ne 0), \ldots, x_n)$

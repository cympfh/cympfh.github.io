...
$X \sim \N(\mu,\sigma^2)$ のとき, 任意の実数 $a$ について,
...
直感的にはこれは誤差 $\epsilon$ の影響を取り除いた $\alpha$ つまり要因の影響の強さを表している.

...
struct Ratio(i64, i64);  // (numerator / denominator)
impl Ratio {
    ...
    fn inv(self) -> Self { Self::new(self.1, self.0) }
    fn gcd(a: i64, b: i64) -> i64 { if b == 0 { a } else { Self::gcd(b, a % b) } }
    fn lcm(a: i64, b: i64) -> i64 { a / Self::gcd(a, b) * b }
}
impl std::ops::Neg for Ratio {
    type Output = Self;
    fn neg(self) -> Self { Ratio(-self.0, self.1) }
}
